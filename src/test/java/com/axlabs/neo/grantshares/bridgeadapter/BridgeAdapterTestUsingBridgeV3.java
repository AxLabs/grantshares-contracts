...
public class BridgeAdapterTestUsingBridgeV3 {
    ...
    @Test
    @Order(1)
    public void execute_proposal_with_bridge_adapter_gas_to_bridge_v3() throws Throwable {
        GasToken gasToken = new GasToken(neow3j);
        BigInteger bridgeFee = bridgeV3.callFunctionReturningInt("nativeDepositFee");
        ...
        BigInteger bridgeBalanceBefore = gasToken.getBalanceOf(bridgeV3.getScriptHash());

        // Data from Neo X
        Account proposer = bob;
        Hash160 recipient = alice.getScriptHash();
        BigInteger amount = gasToken.toFractions(BigDecimal.TEN);
        String offchainUri = "execute_proposal_with_bridge_adapter_gas_v3";
                ...
                ContractSigner.calledByEntry(bridgeAdapter.getScriptHash()).setRules(
                        new WitnessRule(WitnessAction.ALLOW,
                                new AndCondition(new ScriptHashCondition(gasToken.getScriptHash()),
                                        new CalledByContractCondition(bridgeV3.getScriptHash())
                                )
                        ))
        ).sign().send().getSendRawTransaction().getHash();
        waitUntilTransactionIsExecuted(tx, neow3j);
        ...
        BigInteger bridgeBalanceAfter = gasToken.getBalanceOf(bridgeV3.getScriptHash());
        ...
        assertThat(n5.getState().getList().get(1).getAddress(), is(bridgeV3.getScriptHash().toAddress()));
        ...
        assertThat(n6.getState().getList().get(1).getAddress(), is(bridgeV3.getScriptHash().toAddress()));
        assertThat(n6.getState().getList().get(2).getInteger(), is(amount));

        Notification n7 = n.get(6); // Deposit event
        assertThat(n7.getContract(), is(bridgeV3.getScriptHash()));
    ...
    @Test
    @Order(2)
    public void execute_proposal_with_bridge_adapter_neo_to_bridge_v3() throws Throwable {
        GasToken gasToken = new GasToken(neow3j);
        NeoToken neoToken = new NeoToken(neow3j);
        BigInteger bridgeFee = bridgeV3.callFunctionReturningInt("tokenDepositFee", hash160(neoToken.getScriptHash()));
        ...
        BigInteger bridgeBalanceBeforeNeo = neoToken.getBalanceOf(bridgeV3.getScriptHash());
        ...
        BigInteger bridgeBalanceBeforeGas = gasToken.getBalanceOf(bridgeV3.getScriptHash());

        // Data from Neo X
        Account proposer = charlie;
        Hash160 recipient = eve.getScriptHash();
        BigInteger amount = BigInteger.TEN;
        String offchainUri = "execute_proposal_with_bridge_adapter_neo_v3";
        int linkedProposal = -1;
        ...
        BigInteger bridgeBalanceAfterNeo = neoToken.getBalanceOf(bridgeV3.getScriptHash());
        ...
        BigInteger bridgeBalanceAfterGas = gasToken.getBalanceOf(bridgeV3.getScriptHash());
