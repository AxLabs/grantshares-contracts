...
public class TestHelper {
    ...
    public static ContractParameter prepareDeployParameter(Account... members) {
        return array(array(Arrays.stream(members).map(m -> publicKey(m.getECKeyPair().getPublicKey().getEncoded(true)))
                        .collect(Collectors.toList())),
                array(ParameterNames.REVIEW_LENGTH_KEY, ParameterValues.PHASE_LENGTH * 1000,
                        ParameterNames.VOTING_LENGTH_KEY, ParameterValues.PHASE_LENGTH * 1000,
                        ParameterNames.TIMELOCK_LENGTH_KEY, ParameterValues.PHASE_LENGTH * 1000,
    ...
    private static int sendAndEndorseProposal(GrantSharesGovContract gov, Neow3j neow3j, Account proposer,
        ...
        int id = neow3j.getApplicationLog(tx).send().getApplicationLog().getExecutions().get(0).getStack().get(0)
                .getInteger().intValue();

        // 2. endorse proposal
        tx = gov.invokeFunction(GovernanceMethods.ENDORSE, integer(id), hash160(endorser))
                .signers(AccountSigner.calledByEntry(endorser)).sign().send().getSendRawTransaction().getHash();
        Await.waitUntilTransactionIsExecuted(tx, neow3j);

        return id;
    }

    public static void voteForProposal(GrantSharesGovContract gov, Neow3j neow3j, int id, Account endorserAndVoter)
            throws Throwable {
        Hash256 tx = gov.invokeFunction(GovernanceMethods.VOTE, integer(id), integer(1), hash160(endorserAndVoter))
                .signers(AccountSigner.calledByEntry(endorserAndVoter)).sign().send().getSendRawTransaction().getHash();
        Await.waitUntilTransactionIsExecuted(tx, neow3j);
    }

    public static void voteForProposal(GrantSharesGovContract gov, Neow3j neow3j, int id, int vote,
            Account endorserAndVoter)
            throws Throwable {
        Hash256 tx = gov.invokeFunction(GovernanceMethods.VOTE, integer(id), integer(vote), hash160(endorserAndVoter))
                .signers(AccountSigner.calledByEntry(endorserAndVoter)).sign().send().getSendRawTransaction().getHash();
        Await.waitUntilTransactionIsExecuted(tx, neow3j);
    }

    public static Account createMultiSigAccount(int threshold, Account... accounts) {
        List<ECKeyPair.ECPublicKey> pubKeys = Arrays.stream(accounts).map(a -> a.getECKeyPair().getPublicKey())
                .collect(Collectors.toList());
    ...
    public static class GovernanceMethods {
